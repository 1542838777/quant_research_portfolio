...
def get_hfq(stock_codes, start_date, end_date, column):
    ...
    if start_date is not None:
        clode_hfq = clode_hfq[
            clode_hfq.index >= pd.to_datetime(start_date)]
    if end_date is not None:
        clode_hfq = clode_hfq[
            clode_hfq.index <= pd.to_datetime(end_date)]
    if stock_codes is not None:
        clode_hfq = clode_hfq[stock_codes]
    return clode_hfq
...
def load_volatility_data():
    ...
    # 步骤二：加载最原始、最纯净的数据
    # =================================================================
    print("\n1&2. 正在加载后复权 开盘/收盘价 宽表...")
    # 假设 get_hfq(None,...) 返回所有股票的数据
    open_hfq = get_hfq(stock_codes, buffer_START_DATE, END_DATE, 'open')
    close_hfq = get_hfq(stock_codes, buffer_START_DATE, END_DATE, 'close')

    # =================================================================
    # 步骤三：用最基础的方式，一步步重现计算流程
    # =================================================================

    # 4. 计算原始波动率因子 (raw_volatility)
    raw_volatility = close_hfq.pct_change().rolling(window=120, min_periods=60).std() * np.sqrt(252)

    # 5. 计算正确的O2C未来收益率
    end_price = close_hfq.shift(-(FORWARD_PERIOD - 1))
    start_price = open_hfq
    forward_return_o2c = (end_price / start_price - 1)

    # 6. 【关键】执行 T-1 因子对齐
    aligned_factor_wide = raw_volatility.shift(1)
    print("   因子对齐完成。")

    # =================================================================
    # 步骤四：【核心修正】使用 .stack() 进行安全的宽表转长表与合并
    # =================================================================

    print("\n7. 正在使用 .stack() 安全地合并数据...")

    # 7.1 将对齐后的因子宽表转换为长Series，索引为(date, stock)
    factor_long = aligned_factor_wide.stack().rename('factor')

    # 7.2 将收益率宽表转换为长Series
    return_long = forward_return_o2c.stack().rename('return')

    # 7.3 使用 pd.concat 在长表格式上合并，这是最安全、最清晰的方式
    merged_df = pd.concat([factor_long, return_long], axis=1)
    merged_df=merged_df.dropna(inplace=False)
    ...
    print("\n8. 正在计算最终的Spearman相关性...")
    # 在长表上直接计算两列的相关性
    final_correlation = merged_df['factor'].corr(merged_df['return'], method='spearman')

    ####保存数据快照
    aligned_factor_wide = aligned_factor_wide.loc[START_DATE:END_DATE]
    aligned_factor_wide.to_csv('D:\\lqs\\codeAbout\\py\\Quantitative\\quant_research_portfolio\\tests\\workspace\\local_volatility.csv')
    close_hfq = close_hfq.loc[START_DATE:END_DATE]
    close_hfq.to_csv('D:\\lqs\\codeAbout\\py\\Quantitative\\quant_research_portfolio\\tests\\workspace\\local_close_hfq.csv')

    open_hfq = open_hfq.loc[START_DATE:END_DATE]
    open_hfq.to_csv('D:\\lqs\\codeAbout\\py\\Quantitative\\quant_research_portfolio\\tests\\workspace\\local_open_hfq.csv')

    forward_return_o2c = forward_return_o2c.loc[START_DATE:END_DATE]
    forward_return_o2c.to_csv('D:\\lqs\\codeAbout\\py\\Quantitative\\quant_research_portfolio\\tests\\workspace\\local_forward_return_o2c.csv')

    print("\n--- 最终调试结果 V2.0 ---")
    print(f"Spearman Correlation: {final_correlation}")
def  compare_local_mem():
    ...
    mem_open_hfq = pd.read_csv('D:\\lqs\\codeAbout\\py\\Quantitative\\quant_research_portfolio\\tests\\workspace\\mem_open_hfq.csv', index_col=[0], parse_dates=True)
    local_open_hfq = pd.read_csv('D:\\lqs\\codeAbout\\py\\Quantitative\\quant_research_portfolio\\tests\\workspace\\local_open_hfq.csv', index_col=[0], parse_dates=True)

    # 找出不一致的地方
    diff_close_hfq = (local_close_hfq - mem_close_hfq).abs()
    ...
    print(f"difference_fac两个文件的差异总和: {difference_fac.sum().sum()}")
    print(f"两个文件的差异总和: {difference_fac.sum().sum()}")
if __name__ == '__main__':
    # load_volatility_data()

    compare_local_mem()
    t_daily('003017.SZ', '20231105', '20250705', 'vol')
    get_hfq('003017.SZ', '20231105', '20250705', 'close')
    get_hfq('003017.SZ', '20231105', '20250705', 'high')
    get_hfq('003017.SZ', '20231105', '20250705', 'low')
    get_hfq('003017.SZ', '20231105', '20250705', 'open')
    get_hfq('003017.SZ', '20231105', '20250705', 'pct_chg')  # 小数点不准确！
    get_hfq('003017.SZ', '20231105', '20250705', 'amount')
