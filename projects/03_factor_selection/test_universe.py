...
def test_universe():
    ...
    try:
        ...
        # 显示股票池统计信息
        print("\n股票池统计信息:")
        print(f"时间范围: {universe_df.index[0]} 到 {universe_df.index[-1]}")
        print(f"股票数量: {len(universe_df.columns)}")
        ...
        # 显示前几天的股票池情况
        print("\n前5天股票池情况:")
        for i in range(min(5, len(universe_df))):
            date = universe_df.index[i]
            count = universe_df.iloc[i].sum()
