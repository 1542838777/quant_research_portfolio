...
try:
    ...
    try:
        ...
        raise ImportError(f"无法导入必要的模块: {e}. 请确保在正确的目录下运行脚本。")
...
class StrategyFactory:
    ...
    def test_single_factor(self, 
        ...
        if category is None and auto_register:
            returns_data = self.single_factor_tester.get_returns_data()
    ...
    def batch_test_factors(self, 
                          ...
                          auto_register: bool = True,
                          category_mapping: Dict[str, Union[str, FactorCategory]] = None,
                          **test_kwargs) -> Dict[str, Any]:
        """
        批量测试因子
        ...
        if auto_register:
            ...
            for factor_name, factor_data in factor_data_dict.items():
                # 获取类别
                if factor_name in category_mapping:
                    category = category_mapping[factor_name]
                else:
                    # 自动分类
                    category = self.factor_manager.classify_factor(factor_data, returns_data)
                ...
                self.register_factor(
                    name=factor_name,
                    category=category,
                    description=f"批量注册的{category.value if isinstance(category, FactorCategory) else str(category)}因子",
                    data_requirements=["price", "returns"]
                )
