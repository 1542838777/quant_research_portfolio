...
def calculate_advanced_metrics(summary_row: Union[pd.Series, dict]) -> Dict:
    ...
    # 1. 因子独特性：基于与市场的相关性
    market_corr = abs(get_metric(summary_row, 'market_correlation_o2c', 0.4))
    factor_uniqueness = max(0, 1 - market_corr)
    
    # 2. 市场环境适应性：基于IC稳定性
    ic_mean_abs = abs(get_metric(summary_row, 'ic_mean_processed_o2c', 0))
    ic_std = get_metric(summary_row, 'ic_std_processed_o2c', 0.1)
    ...
    # 3. IC一致性：基于胜率的改进版本
    win_rate = get_metric(summary_row, 'tmb_win_rate_processed_o2c', 0.5)
...
def calculate_factor_score_v33(summary_row: Union[pd.Series, dict], 
    ...
    # === 1. 基础指标提取 ===
    base_metrics = {
        'ic_mean': get_metric(summary_row, 'ic_mean_processed_o2c'),
        'ic_ir': get_metric(summary_row, 'ic_ir_processed_o2c'), 
        'fm_t': get_metric(summary_row, 'fm_t_statistic_processed_o2c'),
        'sharpe': get_metric(summary_row, 'tmb_sharpe_processed_o2c'),
        'max_drawdown': get_metric(summary_row, 'tmb_max_drawdown_processed_o2c'),
        'monotonicity': get_metric(summary_row, 'monotonicity_spearman_processed_o2c'),
        'ic_decay': get_metric(summary_row, 'ic_decay_o2c', 0.5),
        'sharpe_raw': get_metric(summary_row, 'tmb_sharpe_raw_o2c')
    }
...
if __name__ == "__main__":
    ...
    # 测试数据
    test_data = {
        'ic_mean_processed_o2c': 0.042,
        'ic_ir_processed_o2c': 0.48,
        'fm_t_statistic_processed_o2c': 2.6,
        'tmb_sharpe_processed_o2c': 0.75,
        'tmb_max_drawdown_processed_o2c': -0.28,
        'tmb_win_rate_processed_o2c': 0.58,
        'monotonicity_spearman_processed_o2c': 0.62,
        'ic_decay_o2c': 0.22,
        'tmb_sharpe_raw_o2c': 0.95,
        'market_correlation_o2c': 0.35,
        'ic_std_processed_o2c': 0.09
    }
