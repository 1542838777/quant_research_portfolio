            ...
            "cells": [
                ...
                },
                {
                    ...
                    "source": [
                        ...
                        "\n",
                        f"- **因子名称**: {factor_name}\n",
                        f"- **因子描述**: {self.config.get('target_factors_for_evaluation', {}).get('description', '无描述')}\n",
                        f"- **数据来源**: {', '.join(self.config.get('target_factors_for_evaluation', {}).get('fields', []))}\n",
                ...
                },
                {
                    ...
                    "source": [
                        ...
                        "### 2.1 股票池构建\n",
                        "\n",
                        f"- **基础股票池**: {self.config.get('stack_pool', {}).get('base', '')}\n",
                        "- **过滤条件**:\n",
                        f"  - 剔除ST股票: {self.config.get('stack_pool', {}).get('filters', {}).get('remove_st', False)}\n",
                        f"  - 最小上市天数: {self.config.get('stack_pool', {}).get('filters', {}).get('min_list_days', 0)}天\n",
                        f"  - 流动性过滤: 剔除后{self.config.get('stack_pool', {}).get('filters', {}).get('min_liquidity_percentile', 0)*100:.0f}%\n",
                        "\n",
                        "### 2.2 因子预处理\n",
                        "\n",
    ...
    def _format_quantile_results_for_notebook(self, qt_results: Dict) -> List[str]:
        ...
        for period_key, result in qt_results.items():
            if 'error' not in result:
                ...
                monotonicity_spearman = result.get(monotonicity_spearman, False)
                
                lines.append(
                    f"| {period_key} | {long_short_return:.4f} | {long_short_sharpe:.4f} | "
                    f"{monotonicity_spearman} |\n"
                )
        ...
        for period_key, result in qt_results.items():
            if 'error' not in result:
                qt_data.append({
                    ...
                    '单调系数':  result.get('monotonicity_spearman')
                })
